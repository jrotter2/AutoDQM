...
def compile_histpairs(config_dir, subsystem,
    ...
    for hconf in conf_list:
        ...
        if "*" not in h:
             if h in [str(keys)[2:-3] for keys in data_keys_uproot] and h in [str(keys)[2:-3] for keys in ref_keys_uproot]:
            ...
            for name in data_keys_uproot:
                if h.split("*")[0] in str(name) and name in ref_keys_uproot and not "<" in str(name):
                    valid_names.append(name.decode('utf8')[:-2])
        ...
        for name in valid_names:
            ...
            if "xlabels" in hconf.keys():
                data_hist_uproot._fXaxis._fLabels = hconf["xlabels"]
                ref_hist_uproot._fXaxis._fLabels = hconf["xlabels"]
            if "ylabels" in hconf.keys():
                data_hist_uproot._fYaxis._fLabels = hconf["ylabels"]
                ref_hist_uproot._fYaxis._fLabels = hconf["ylabels"]
